package org.learn.index12306.biz.ticketservice.job.base;
...
public abstract class AbstractTrainStationJobHandlerTemplate extends IJobHandler {
    ...
    public void execute() throws Exception {
        ...
        var dataTime = StrUtil.isNotBlank(requestParam) ? DateUtil.parse(requestParam, "yyyy-MM-dd") : DateUtil.tomorrow();
        var trainMapper = ApplicationContextHolder.getBean(TrainMapper.class);
        for(;; currentPage++){
            var queryWrapper = Wrappers.lambdaQuery(TrainDO.class)
                    .between(TrainDO::getDepartureTime, DateUtil.beginOfDay(dataTime), DateUtil.endOfDay(dataTime));
